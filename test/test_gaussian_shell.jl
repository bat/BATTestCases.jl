...
@testset "Gaussian Shell Distribution" begin
    ...
    @test @inferred(GaussianShell(r = 3., w = 0.1, n = 3)) isa GaussianShell
    @test @inferred(GaussianShell()) isa GaussianShell

    gs = GaussianShell(r = 3., w = 0.1, n = 3)
    ...
    # ToDo: Re-enable once sampling is implemented again:
    # @test isapprox(
    #    @inferred(Statistics.cov(gs)),
    #    [3.02718 0.00166016 0.0113455; 0.00166016 3.01293 0.00453035; 0.0113455   0.00453035  3.01046],
    #    atol = 1e-1
    #)
    ...
    gs = GaussianShell(r = 5e-10, w = 1., n = 1)
    
    # ToDo: Re-enable once sampling is implemented again:
    # ks_test = HypothesisTests.ExactOneSampleKSTest(rand(gs, 10^6)[:], Normal(0, 1.))
    # @test pvalue(ks_test) > 0.001 # ToDo: Try to increase to 0.05
end
